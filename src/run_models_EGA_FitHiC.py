...
Arguments:
    ...
    '-hm', '--histone_modifications',  default=5, type=int
...
def train_model_classification(model, graph, max_epoch, learning_rate, targetNode_mask, train_idx, valid_idx, optimizer):
    ...
    eprint('\n# Epoch 0 out of', str(max_epoch))
    for e in list(range(max_epoch)):
        
        if (e+1)%100 == 0:
            eprint('# Epoch', str(e+1), 'out of', str(max_epoch))
...
def eval_model_classification(model, graph, targetNode_mask, train_idx, valid_idx, test_idx):
    ...
    return test_AUROC, test_AUPR, test_pred, test_labels, train_AUPR, train_pred, train_labels, \
        valid_AUPR, valid_pred, valid_labels, test_softmax
...
#-c MEF -rf 0 -hm 2 -cr 10000 -df data
parser = argparse.ArgumentParser()

parser.add_argument('-c',  '--cell_line',                    default='B72',  type=str)
...
parser.add_argument(       '--noModel',                      action='store_false')
...
save_model            = args.noModel


num_feat = int((hic_res/chip_res)*num_hm)

num_classes = 1            if regression_flag == 1 else 2
task        = 'Regression' if regression_flag == 1 else 'Classification'
...
###Load input files
base_path               = os.getcwd()
save_dir                = os.path.join(base_path, data_folder, cell_line, 'saved_runs')
...
#     Split Train/Dev/Test: -/chr5/chr4
# 
# {' '*18}Hyperparameters
# {'-'*50}
#                   Epochs: {max_epoch}
#            Learning Rate: {learning_rate}
# No. of Graph Conv Layers: {str(num_graph_conv_layers)}
#           Embedding size: {graph_conv_embed_size}
#     No. of Linear Layers: {num_lin_layers}
# Linear Hidden Layer Size: {lin_hidden_size}"""
...
# geneLabs           = geneNodes_labs[:, -1].astype(int) if regression_flag == 0 else geneNodes_labs[:, -1].astype(float)
geneLabs           = geneNodes_labs[:, -1].astype(int if regression_flag == 0 else float)
allLabs[geneNodes] = geneLabs
Y                  = torch.tensor(allLabs).long()     if regression_flag == 0 else torch.tensor(allLabs).float()

extract = torch_geometric.utils.from_scipy_sparse_matrix(mat)
# Subset to only the regions I found with fithic
...
# Get pd data frame with gene and chr and use it to split data
gene_chr = pd.read_csv('/mnt/BioAdHoc/Groups/RaoLab/Edahi/Projects/09.TimeCourse5hmC/ForFerhatGit/FCDNN/results/cpg_composition_differences/all_genes_promoter_coords.bed', usecols=['chr','gene'], sep="\t")
gene_chr4 = gene_chr.query('chr=="chr4"').gene.tolist()
gene_chr5 = gene_chr.query('chr=="chr5"').gene.tolist()


train_gene_ID = np.array(df_genes.query('abbrev not in @gene_chr4 and abbrev not in @gene_chr5').hic_node_id.tolist())
valid_gene_ID = np.array(df_genes.query('abbrev in @gene_chr5').hic_node_id.tolist())
test_gene_ID  = np.array(df_genes.query('abbrev in @gene_chr4').hic_node_id.tolist())

train_idx = torch.tensor([index for index, value in enumerate(torch.isin(targetNode_mask, torch.tensor(train_gene_ID))) if value])
valid_idx = torch.tensor([index for index, value in enumerate(torch.isin(targetNode_mask, torch.tensor(valid_gene_ID))) if value])
test_idx = torch.tensor([index for index, value in enumerate(torch.isin(targetNode_mask, torch.tensor(test_gene_ID))) if value])
...
if regression_flag == 0:
    ...
    plt.ylabel('TPR')
    plt.xlabel('FPR')
    Title = "Area Under the ROC: " + str(test_AUROC.round(4)) + "\nCelltype = "+ str(cell_line)
    plt.title(Title)
    plt.savefig(os.path.join(save_dir,'model_' + date_and_time + '_AUROC.png'))
    ...
    with open(os.path.join(save_dir,'model_' + date_and_time + '_AUC_FPR_TPR.csv'), "wt") as f:
...
### For regression:
    ...
    plt.title(Title)
    plt.savefig(os.path.join(save_dir,'model_' + date_and_time + '_PCC.png'))
    ### Save metrics and node predictions
    # train
...
# print('\n# Elapsed time: {0:02d}:{1:02d}:{2:02d}'.format(elapsed_h, elapsed_m, elapsed_s))
eTime='{0:02d}:{1:02d}:{2:02d}'.format(elapsed_h, elapsed_m, elapsed_s)

if regression_flag == 0:
    model_results_info = f"""# 
# {' '*20}Results
# {'-'*50}
#               Test AUROC: {round(test_AUROC,4)}
#                Test AUPR: {round(test_AUPR,4)}
# 
# {' '*18}Elapsed Time
# {'-'*50}
#                 HH:MM:SS: {eTime}
...
print(model_results_info)

### Save trained model parameters, model predictions CSV file, model performance/information
if save_model:
    model_path = os.path.join(save_dir, 'FITHIC_model_chr4test_' + date_and_time + '.pt')
    torch.save(model.state_dict(), model_path)
    df_full_metrics_filename = os.path.join(save_dir, 'FITHIC_model_chr4test_predictions_' + date_and_time + '.csv')
    df_full_metrics.to_csv(df_full_metrics_filename, index=False)

model_info_filename = os.path.join(save_dir,'FITHIC_model_chr4test_' + date_and_time + '_info.txt')
f = open(model_info_filename, 'w')
f.write(model_specs_info)
f.write(model_results_info)
f.write('\n\n')
f.close()
