...
def gen_hic_map(hic_matrix_coord, weights_counts, hic_res, kth):
    ...
    chrom_start_local = np.min(hic_matrix_coord[:, 1])
    num_bins_sq = int((np.max(hic_matrix_coord[:, 0]) - np.min(hic_matrix_coord[:, 1]))/hic_res)
    chrom_finish_local = chrom_start_local + num_bins_sq*hic_res
    ...
    hic_map_reset, xedges, yedges = np.histogram2d(hic_matrix_coord[:,1], hic_matrix_coord[:,0], \
        weights = weights_counts, bins = [bin_edges_x, bin_edges_y])
...
parser.add_argument('-c', '--cell_line', default='B00', type=str)
...
base_path = os.getcwd()
#rnaseq_file = os.path.join(base_path, 'data_bkup', 'rnaseq.csv')
#gene_coord_file = os.path.join(base_path, 'data_bkup', 'gene_coord.csv')
#cell_line_dir = os.path.join(base_path , 'data_bkup', cell_line)
...
chr_num_low_inc = 1
#chr_num_high_exc = 23
chr_num_high_exc = args.highest_chromosome_excluding
...
df_node_coord_file = os.path.join(cell_line_dir, 'df_node_coord.pkl')   
