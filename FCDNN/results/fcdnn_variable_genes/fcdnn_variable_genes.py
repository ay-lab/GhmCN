...
# NOTE: Run /mnt/BioAdHoc/Groups/RaoLab/Edahi/Projects/09.TimeCourse5hmC/ForFerhatGit/FCDNN/results/fcdnn_variable_genes/fcdnn_variable_genes.sh before this code
import pandas as pd
import numpy as np
import os
...
for subset in ['Dev','Test','Train']:
    ...
    m_set_genes = pd.read_csv(f'/tmp/any_variable_genes_{subset}.txt',names= ['genes'],sep="\t")
...
np.savez(os.path.join(out_dir, 'CleanSamplesAnyVariable_X.npz'), concatenated_x)
np.savez(os.path.join(out_dir, 'CleanSamplesAnyVariable_Y.npz'), concatenated_y)

# Test loading:
x_data = np.load(os.path.join(out_dir, 'CleanSamplesAnyVariable_X.npz'))['arr_0']
y_data = np.load(os.path.join(out_dir, 'CleanSamplesAnyVariable_Y.npz'))['arr_0']
x_data.shape
y_data.shape
